...
def main(args):
    ...
    def create_curved_edge(pos, node1, node2, k_factor=0.2):
        x1, y1 = pos[node1]
        x2, y2 = pos[node2]
        ...
        midpoint = (mid_x + k_factor * normal_x, mid_y + k_factor * normal_y)
        return [pos[node1], midpoint, pos[node2]]
    ...
    with torch.no_grad():
        ...
        for idx in keep_queries:
            u = CLASSES[probas_sub[idx].argmax()]
            v = CLASSES[probas_obj[idx].argmax()]
            label = REL_CLASSES[probas[idx].argmax()]
        ...
        G.add_edges_from((u, v, {'label': label}) for u, v, label in edgeList)     
